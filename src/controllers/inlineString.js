...
export const inlineStyleAffectAttribute = {
  bl: 1, it: 1, ff: 1, cl: 1, un: 1, fs: 1, fc: 1,
};
export const inlineStyleAffectCssName = {
  'font-weight': 1, 'font-style': 1, 'font-family': 1, 'text-decoration': 1, 'border-bottom': 1, 'font-size': 1, color: 1,
};
...
export function isInlineStringCT(ct) {
  const isIs = ct != null && ct.t == 'inlineStr' && ct.s != null && ct.s.length > 0;
  return isIs;
}
...
export function updateInlineStringFormat(cell, attr, value, $input) {
  ...
  if ($textEditor.length > 0) {
    if (startContainer === endContainer) {
      const span = startContainer.parentNode; let spanIndex; let
        inherit = false;

      const content = span.innerHTML;

      const fullContent = $textEditor.html();
      ...
      let mid = '';
      let right = '';
      const s1 = 0; const s2 = startOffset; const s3 = endOffset; const
        s4 = content.length;
      ...
      if (left != '') {
        ...
        if (inherit) {
          const box = $(span).closest('#luckysheet-input-box').get(0);
          if (box != null) {
      ...
      const startSpan = startContainer.parentNode; let
        startSpanIndex;
      const endSpan = endContainer.parentNode; let
        endSpanIndex;

      startSpanIndex = $textEditor.find('span').index(startSpan);
      endSpanIndex = $textEditor.find('span').index(endSpan);

      const startContent = startSpan.innerHTML; const
        endContent = endSpan.innerHTML;
      let sleft = '';
      let sright = '';
      let eleft = '';
      let eright = '';
      const s1 = 0; const s2 = startOffset; const s3 = endOffset; const
        s4 = endContent.length;
...
export function enterKeyControll(cell) {
  ...
  const $functionbox = $(cac).closest('#luckysheet-functionbox-cell');
  ...
  if ($textEditor.length > 0) {
    ...
    const startContent = startSpan.innerText;
    let sleft = '';
    let sright = '';
    const s1 = 0; const
      s2 = startOffset;
    ...
    let spanIndex; let cont;
    if (startContainer.parentNode.tagName == 'SPAN') {
      const textSpan = $textEditor.find('span');
      spanIndex = textSpan.index(startSpan);
      if ((spanIndex == textSpan.length - 1) && sright == '') {
        const txt = textSpan[spanIndex].innerHTML;
      ...
      const cssText = getFontStyleByCell(cell);
      if (sright == '') {
...
export function convertSpanToShareString($dom) {
  const styles = []; let preStyleList; let
    preStyleListString = null;
  for (let i = 0; i < $dom.length; i++) {
    const span = $dom.get(i);
...
function getCssText(cssText, attr, value) {
  ...
  const ukey = textTrim(s.substr(0, s.indexOf(':')));
  let uvalue = textTrim(s.substr(s.indexOf(':') + 1));
  uvalue = uvalue.substr(0, uvalue.length - 1);
  // let cssText = span.style.cssText;
  cssText = removeClassWidthCss(cssText, attr);
